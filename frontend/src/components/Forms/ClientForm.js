...
function ClientForm() {
    const [fields, setFields] = useState({ clientName: '', clientUserName: '', password: '', clientEmail: '', dateOfJoining: '', logo: { filename: '' }, sendMail: 'no' })
    ...
    const handleClear = () => {
        setFields({ clientName: '', clientUserName: '', password: '', clientEmail: '', dateOfJoining: '', logo: { filename: '' }, sendMail: 'no' })
        setLogo([])
    }
    ...
    const handleSubmit = (e) => {
        ...
        form.append('clientData', JSON.stringify(fields))
        const api = Instance()
        api.post('/api/addclientdetails', form,{
            headers:{
                        ...
                        <Paper sx={{ p: 3, }} elevation={20}  >
                            <Stack component={'form'} onSubmit={handleSubmit} spacing={{ xs: 2, xl: 3 }}>
                                ...
                                <Stack spacing={2} sx={{ width: '100%' }} direction={'row'} display={'flex'} alignItems={'center'}>
                                    ...
                                    >
                                        ...
                                        <VisuallyHiddenInput type="file" accept="image/*" max={1} onInput={async (e) => {
                                            ...
                                            try {
                                                ...
                                                // file['url'] = url
                                                // console.log(url)
                                                setFields({ ...fields, logo: { filename: file[0].name, } })
                                            }
                                            catch (err) {
                                                // console.log(err)
                                            }
                                    ...
                                    <Fade in={fields.logo.filename !== ''} timeout={'auto'}>
                                        <Typography component={'h5'} variant='p'>File: {fields.logo.filename}<IconButton size='small' onClick={
                                            () => {
                                                setFields({ ...fields, logo: { filename: '', } }) 
                                                setLogo([])
                                                }}>
                                            <Cancel fontSize='12px' color='error' /></IconButton></Typography>
                                    </Fade>
                                </Stack>
                                <FormControl >
                                    <RadioGroup
