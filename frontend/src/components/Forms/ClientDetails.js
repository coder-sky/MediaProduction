...
function ClientDetails() {
    ...
    const [editFields, setEditFields] = useState({ client_name: '', username: '', email: '', date_of_joining: '', logo: { filename: '' } })
    ...
    const handleClose = () => {
        setOpen(false);
        setEditFields({ client_name: '', username: '', email: '', date_of_joining: '', logo: { filename: '' }})
        setPrevEditFields({ client_name: '', username: '', email: '', date_of_joining: '', logo: { filename: '' }})
        setLogo([])
    };
    ...
    const imageBodyTemplate = (rowData) => {
        return <img src={process.env.REACT_APP_BACKEND_SERVER+rowData.company_logo} alt={rowData.client_name} onError={(err) => err.target.src = 'noImage.jpeg'} width="64px" height={'40px'} style={{ objectFit: 'contain' }} className="shadow-4" />;
    };
    ...
    const handleEdit = (rowData) => {
        ...
        setEditFields({ ...rowData, logo: { filename: filename } })
        setPrevEditFields({ ...rowData, logo: { filename: filename} })
        setLogo([])
        setOpen(true)
    ...
    const editMode = useMemo(() => {
        ...
        const handleSubmit = async(e)=>{
            ...
            if(JSON.stringify(prevEditFields)!==JSON.stringify(editFields)){
                ...
                form.append('clientLogo', logo[0])
                form.append('newData', JSON.stringify(editFields))
                form.append('prevData', JSON.stringify(prevEditFields))
                // console.log(logo)
                try{
                    const api = Instance()
                    const res = await api.put('/api/editclient', form,{
                        headers:{
                ...
                >
                    ...
                    <DialogContent>
                        <Stack component={'form'} onSubmit={handleSubmit} id='editclientform' spacing={{ xs: 2, xl: 3 }} p={1}>
                            ...
                            >
                                ...
                                <VisuallyHiddenInput type="file" accept="image/*" max={1} onInput={async (e) => {
                                    ...
                                    try {
                                        ...
                                        // file['url'] = url
                                        // console.log(url)
                                        setEditFields({ ...editFields, logo: { filename: file[0].name,} })
                                    }
                                    catch (err) {
                                        // console.log(err)
                                    }
                            ...
                            <Collapse in={editFields.logo.filename !== ''} unmountOnExit timeout={'auto'} >
                                <Typography component={'h5'} variant='p'>File: {editFields.logo.filename}<IconButton size='small' onClick={() => setEditFields({ ...editFields, logo: { filename: '',} })}><Cancel fontSize='12px' color='error' /></IconButton></Typography>
                            </Collapse>
    ...
    }, [editFields, open,prevEditFields,loadButton,logo])
